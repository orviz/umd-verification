...
class InfoModel(object):
	...
	if summary:
	    ...
	    raise exception.InfoModelException("Cannot parse glue-validator output: %s" % r)

    def _run_version_check(self):
        conn = ldap.initialize("ldap://localhost:2170")
        try:
	    ldap_result = conn.search_s("GLUE2GroupID=resource,o=glue",
                                        ldap.SCOPE_SUBTREE,
	    ...
	    base_utils.to_file("/tmp/qc_info_3.out",
            ...
            for dn, attrs in ldap_result:
                try:
                    ...
                    return False, "No implementation version found for DN: %s" % dn
                try:
                    d = dict([attr.split('=', 1)
